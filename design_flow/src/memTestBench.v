...
// synopsys translate_off
	...
	// read input from file
		localparam rsize = 1024;
		vcReadFile #(.size(rsize), .filename("localmem_write.txt"), .hwidth(hwidth))
	...
	// write output data to file
		...
		*/
		...
		localparam wsize = 1;
		vcWriteFile #(.size(wsize), .filename("localmem_read.txt"), .hwidth(hwidth))
	...
	// start generating VCD file for power estimation
		...
		//================================================
			localparam		mem_width	 	= 64*3*8;			// equal the size of FFT64 = 64*3B*8b = 1536	bits
			localparam		mem_hwidth	= mem_width/2;	// equal the size of FFT32 = 32*3B*8b = 768 	bits

			wire 			[15:0]						CE1, CE2;
			wire 										WEB1, OEB1, CSB1, WEB2, OEB2, CSB2;
			reg	  signed	[6:0]						A1, A2;
			wire  signed  	[mem_width-1: 0]			I1, I2, O1, O2;		
			...
			assign {dt_out127, dt_out126, dt_out125, dt_out124, dt_out123, dt_out122, dt_out121, dt_out120, 
					dt_out119, dt_out118, dt_out117, dt_out116, dt_out115, dt_out114, dt_out113, dt_out112, 
					dt_out111, dt_out110, dt_out109, dt_out108, dt_out107, dt_out106, dt_out105, dt_out104, 
					dt_out103, dt_out102, dt_out101, dt_out100, dt_out99, dt_out98, dt_out97, dt_out96, 
					dt_out95, dt_out94, dt_out93, dt_out92, dt_out91, dt_out90, dt_out89, dt_out88, 
					dt_out87, dt_out86, dt_out85, dt_out84, dt_out83, dt_out82, dt_out81, dt_out80, 
					dt_out79, dt_out78, dt_out77, dt_out76, dt_out75, dt_out74, dt_out73, dt_out72, 
					dt_out71, dt_out70, dt_out69, dt_out68, dt_out67, dt_out66, dt_out65, dt_out64, 
					dt_out63, dt_out62, dt_out61, dt_out60, dt_out59, dt_out58, dt_out57, dt_out56, 
					dt_out55, dt_out54, dt_out53, dt_out52, dt_out51, dt_out50, dt_out49, dt_out48, 
					dt_out47, dt_out46, dt_out45, dt_out44, dt_out43, dt_out42, dt_out41, dt_out40, 
					dt_out39, dt_out38, dt_out37, dt_out36, dt_out35, dt_out34, dt_out33, dt_out32, 
					dt_out31, dt_out30, dt_out29, dt_out28, dt_out27, dt_out26, dt_out25, dt_out24, 
					dt_out23, dt_out22, dt_out21, dt_out20, dt_out19, dt_out18, dt_out17, dt_out16, 
					dt_out15, dt_out14, dt_out13, dt_out12, dt_out11, dt_out10, dt_out9, dt_out8, 
					dt_out7, dt_out6, dt_out5, dt_out4, dt_out3, dt_out2, dt_out1, dt_out0} = O2;
			...
			//--- initialize for memory control signals
				...
				assign CSB1 	= !rst;		
				assign OEB1 	= 0;		
				assign WEB1	= stall;			// write port 1

				assign CSB2 	= !rst;	
				assign OEB2 	= 0;	
				assign WEB2	= 1;			// read  port 2
				...
				always @(posedge clk or posedge rst) begin
					if (rst) begin
						...
						A2 <= -1;
						read_cnt <= 0;
						$display("INFOR --- Port 1: CE1=%d, CSB1=%d, OEB1=%d, WEB1=%d, A1=%d", CE1, CSB1, OEB1, WEB1, A1);
						$display("INFOR --- Port 2: CE2=%d, CSB2=%d, OEB2=%d, WEB2=%d, A2=%d", CE2, CSB2, OEB2, WEB2, A2);
					end
					else begin
						if (!stall_out) begin
							A1 <= A1 + 1;
							...
							if (read_cnt >10) begin
								$display("WARNS --- End of simulation !!!");
								$display("INFOR --- Port 1: CE1=%d, CSB1=%d, OEB1=%d, WEB1=%d, A1=%d", CE1, CSB1, OEB1, WEB1, A1);
								$display("INFOR --- Port 2: CE2=%d, CSB2=%d, OEB2=%d, WEB2=%d, A2=%d", CE2, CSB2, OEB2, WEB2, A2);
								$display("INFOR --- read_cnt = %d", read_cnt);	
								$finish;
							end 
						end
					end 
			...
			// Design-Under-Test (DUT)
				`ifdef  SIM_DMEM
					dmem dut_localmem (	.CE1(CE1[`DMEM_NUM_BANK-1:0]), .WEB1(WEB1), .OEB1(OEB1), .A1(A1[4:0]), .I1(I1), .O1(O1),
											.CE2(CE2[`DMEM_NUM_BANK-1:0]), .WEB2(WEB2), .OEB2(OEB2), .A2(A2[4:0]), .I2(I2), .O2(O2) );
					initial begin
						$display("INFOR --- DMEM:       rows=%d, cols=%d, addr=%d bits",  
								`DMEM_BANK_ROWS, `DMEM_BANK_COLS, `DMEM_ADDR);
						$display("INFOR --- BANK:        num_banks=%d, rows=%d, cols=%d, addr=%d bits",  
								`DMEM_NUM_BANK, `DMEM_BANK_ROWS, `DMEM_BANK_COLS, `DMEM_BANK_ADDR);
						$display("INFOR --- SUB-BANK: num_sub_banks=%d, rows=%d, cols=%d, addr=%d bits",  
								`DMEM_NUM_SUB_BANK, `DMEM_SUB_BANK_ROWS, `DMEM_SUB_BANK_COLS, `DMEM_SUB_BANK_ADDR);

					end
				`elsif  SIM_FMEM
					fmem dut_localmem (	.CE1(CE1[0]), .WEB1(WEB1), .OEB1(OEB1), .CSB1(CSB1),  .A1(A1[4:0]), .I1(I1), .O1(O1),
											.CE2(CE2[0]), .WEB2(WEB2), .OEB2(OEB2), .CSB2(CSB2),  .A2(A2[4:0]), .I2(I2), .O2(O2) );
				`elsif  SIM_TMEM
					tmem dut_localmem (	.CE1(CE1[0]), .WEB1(WEB1), .OEB1(OEB1), .CSB1(CSB1),  .A1(A1[5:0]), .I1(I1[mem_hwidth-1: 0]), .O1(O1[mem_hwidth-1: 0]),
											.CE2(CE2[0]), .WEB2(WEB2), .OEB2(OEB2), .CSB2(CSB2),  .A2(A2[5:0]), .I2(I2[mem_hwidth-1: 0]), .O2(O2[mem_hwidth-1: 0]) );
				`else
					//$display("ERROR --- LOCALMEM is undefined");
					//$finish;
				`endif		
